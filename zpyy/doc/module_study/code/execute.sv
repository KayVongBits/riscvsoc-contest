...
always_comb begin : execute_comb
    unique case (inst_s.opcode)
        U_LUI, U_AUIPC : begin
            inst_u_type = Inst_U_Type_s'(inst_i)    ;
        end 
        J_JAL : begin   
            inst_j_type = Inst_J_Type_s'(inst_i)    ;
        end 
        R_ARI_LOG : begin   
            inst_r_type = Inst_R_Type_s'(inst_i)    ;

        end 
        B_BRANCH : begin    
            inst_b_type = Inst_B_Type_s'(inst_i)    ;
        end 
        S_SAVE : begin  
            inst_s_type = Inst_S_Type_s'(inst_i)    ;
        end
        I_JALR, I_LOAD, I_ARI_LOG,I_FENCE,I_ECALL_CSR : begin
            inst_i_type = Inst_I_Type_s'(inst_i)    ;
