...
module execute import rv32i_pkg::*;(
    ...
    // 处理数据前递
    input   var     Forwarding_Mode_e   rs1_forward_mode_i  ,
    input   var     Forwarding_Mode_e   rs2_forward_mode_i  ,
    input   logic   [`DATA_WIDTH-1:0]   mem_forward_data_i  ,
    input   logic   [`DATA_WIDTH-1:0]   wb_forward_data_i   
...
logic   [`DATA_WIDTH-1:0]           true_rs1_data   ;           // 处理数据前递
logic   [`DATA_WIDTH-1:0]           true_rs2_data   ;           // 处理数据前递
...
always_comb begin
    unique case (rs1_forward_mode_i)
        MEM_FORWARDING  : true_rs1_data = mem_forward_data_i    ;
        WB_FORWARDING   : true_rs1_data = wb_forward_data_i     ;
        default         : true_rs1_data = id2ex_bus_i.rs1_data ;
    endcase

    unique case (rs2_forward_mode_i)
        MEM_FORWARDING  : true_rs2_data = mem_forward_data_i    ;
        WB_FORWARDING   : true_rs2_data = wb_forward_data_i     ;
        default         : true_rs2_data = id2ex_bus_i.rs2_data ;
    endcase
end

assign alu_op1 = ( id2ex_bus_i.ex_ctrl.alu_src1_sel == `ALU_OP1_SEL_RS1 ) ? true_rs1_data : id2ex_bus_i.inst_addr    ;
assign alu_op2 = ( id2ex_bus_i.ex_ctrl.alu_src2_sel == `ALU_OP2_SEL_RS2 ) ? true_rs2_data : id2ex_bus_i.imm          ;
            ...
            unique case (inst_s_type.funct3)
                S_TYPE_SB : begin
                    unique case (ex2mem_bus_o.alu_res[1:0])
                        2'b00 : begin
                            ex2mem_bus_o.rs2_data[7:0]          = true_rs2_data[7:0] ;
                            ex2mem_bus_o.mem_ctrl.ram_wr_mask   = RAM_WR_B_1                ;  
                        end
                        2'b01 : begin
                            ex2mem_bus_o.rs2_data[15:8]         = true_rs2_data[7:0] ;
                            ex2mem_bus_o.mem_ctrl.ram_wr_mask   = RAM_WR_B_2                ;  
                        end
                        2'b10 : begin
                            ex2mem_bus_o.rs2_data[23:16]        = true_rs2_data[7:0] ;
                            ex2mem_bus_o.mem_ctrl.ram_wr_mask   = RAM_WR_B_3                ;  
                        end
                        default : begin
                            ex2mem_bus_o.rs2_data[31:24]        = true_rs2_data[7:0] ;
                ...
                S_TYPE_SH : begin
                    unique case (ex2mem_bus_o.alu_res[1])
                        1'b0 : begin
                            ex2mem_bus_o.rs2_data[15:0]         = true_rs2_data[15:0]    ;
                            ex2mem_bus_o.mem_ctrl.ram_wr_mask   = RAM_WR_HW_1                   ;  
                        end
                        default : begin
                            ex2mem_bus_o.rs2_data[31:16]        = true_rs2_data[15:0]    ;
                            ex2mem_bus_o.mem_ctrl.ram_wr_mask   = RAM_WR_HW_2                   ;
                        end
                    endcase
                end
                S_TYPE_SW : begin
                    ex2mem_bus_o.rs2_data               = true_rs2_data         ;
