...
DAT_DIR  = "./test_data/dat"  # 💡 这里将输出目录改为了 dat 文件夹
...
def batch_convert_and_verify():
    ...
    for filename in dump_files:
        ...
        dat_path = os.path.join(DAT_DIR, f"{test_name}.dat") # 💡 修改了生成的文件后缀为 .dat
        ...
        try:
            with open(dump_path, "r") as f_in, open(dat_path, "w") as f_out: # 💡 写入 .dat 文件
                for line in f_in:
        ...
        actual_codes = []
        with open(dat_path, "r") as f_dat: # 💡 从 .dat 文件读取进行验证
            for line in f_dat:
        ...
        if len(expected_codes) != len(actual_codes):
            print(f"   └─ ❌ 验证: 失败 (数量不匹配 Dump:{len(expected_codes)} DAT:{len(actual_codes)})") # 💡 修改了报错提示文本
            fail_count += 1
            print("-" * 53)
            continue
